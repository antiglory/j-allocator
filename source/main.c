...
void* jalloc(const size_t _Size, const byte_t _Priv) {
    ...
    if (prev != NULL) {
        ...
        page_start = mmap(NULL, f_Size, prot, flags, -1, 0);
        if (page_start == MAP_FAILED) return NULL;
    }

    memset(page_start, 0, sizeof(chunk_t));
...
int main(void) {
    int* heap = jalloc(2 * sizeof(int), PROT_READ_BIT | PROT_WRITE_BIT);
    if (!heap) puts("deu ruim");
    *heap = 10;
